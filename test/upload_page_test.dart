...
void main() {
  ...
  group('UploadPage Comprehensive Tests', () {
    testWidgets('Should handle successful image OCR and auto-save', (
      tester,
    ) async {
    ...
    testWidgets('Should handle successful PDF extraction and auto-save', (
      ...
      when(
        () => mockFileService.extractPdfText(any()),
      ).thenAnswer((_) async => mockPdfText);
      when(
        () => mockReportsCollection.add(any()),
      ).thenAnswer((_) async => _MockDocumentReference());
    ...
    testWidgets('Should show manual entry if OCR text has no metrics', (
      tester,
    ) async {
      tester.view.physicalSize = const Size(1200, 1600);
      tester.view.devicePixelRatio = 1.0;
      ...
      when(
        () => mockFileService.recognizeImage(any(), any()),
      ).thenAnswer((_) async => 'No metrics here');
