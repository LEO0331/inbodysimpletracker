...
void main() {
  ...
  setUp(() {
    ...
    ).thenAnswer((_) => Stream.value(usersSnapshot));
    ...
    when(
      () => userDoc.data(),
    ).thenReturn({'email': 'test@user.com', 'role': 'user'});
    ...
    when(
      () => reportsCollection.orderBy('reportDate', descending: true),
    ).thenReturn(reportsCollection);
    when(
      () => reportsCollection.snapshots(),
    ).thenAnswer((_) => Stream.value(reportsSnapshot));
